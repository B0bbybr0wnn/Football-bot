...
# Top competitions to pull fixtures from
COMPETITIONS = ["PL", "CL", "PD", "SA", "BL1", "FL1"]  # EPL, UCL, La Liga, Serie A, Bundesliga, Ligue 1
...
def fetch_fixtures():
    ...
    for comp in COMPETITIONS:
        try:
            ...
            if r.status_code != 200:
                ...
                continue
            matches = r.json().get("matches", [])[:5]  # top 5 per comp
            for m in matches:
    ...
    return "\n".join(all_fixtures[:10]) if all_fixtures else "No upcoming fixtures available."
...
def fetch_news(topic, posted):
    ...
    source = random.choice(top_items[:3])["link"]
...
- 2-3 punchy paragraphs previewing 2-3 of the biggest upcoming matches
- Mention actual team names and kickoff days
